...
form.addEventListener("demo-submit", function(e){
    ...
    if (firstName.value.length < 3){console.log(`${firstName.value.length} is less than 3`);}
    ...
    if (lastName.value.length < 3){console.log(`${lastName.value.length} is less than 3`);}
        // if(firstName.value === "" || lastName.value === ""){ // Original conditional
    if (firstName.value.length < 3 || lastName.value.length < 3){ // (Me) A custom conditional
        const errorMessage = document.createElement("p")
        errorMessage.style.color = "red";
        errorMessage.style.fontWeight = 800;
        errorMessage.innerText = "Sorry - all fields must have an entry that is at least 3 characters-long";
